...
def parse_args():
    ...
    # ask if to do on gpu or cpu
    parser.add_argument('--gpu',type=str, help='Use GPU for processing y/n')
...
def merge_channels_cpu(file_lists, channels):
    # Merge the channels
    for i in tqdm(range(len(file_lists[channels[0]])), desc='Merging channels'):
        ...
        merged_img = np.zeros((height, width, 0), dtype=np.uint8)
        ...
        merged_img = np.moveaxis(merged_img, source=-1, destination=0)
        # save the merged image
        imwrite(os.path.join(merged_dir, os.path.basename(file_lists[channels[0]][i]).replace(channels[0],'')), merged_img, compression='zlib')

    print("Merged images saved in", merged_dir)

def merge_channels_gpu(file_lists, channels):
    # Merge the channels
    for i in tqdm(range(len(file_lists[channels[0]])), desc='Merging channels'):
        # create an empty merged image
        img = imread(file_lists[channels[0]][i])
        ...
        # save the merged image
        imwrite(os.path.join(merged_dir, os.path.basename(file_lists[channels[0]][i]).replace(channels[0],'')), cp.asnumpy(merged_img), compression='zlib')

    print("Merged images saved in", merged_dir)

    
if args.gpu == 'y':
    merge_channels_gpu(file_lists, channels)
elif args.gpu == 'n':
    merge_channels_cpu(file_lists, channels)
else:
